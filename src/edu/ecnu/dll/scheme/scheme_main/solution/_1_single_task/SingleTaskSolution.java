package edu.ecnu.dll.scheme.scheme_main.solution._1_single_task;
...
import edu.ecnu.dll.scheme.scheme_main.struct.agent.worker.SingleTaskPrivacyBasicWorker;
...
public class SingleTaskSolution extends Solution {
    ...
    public SingleTaskPrivacyBasicWorker[] workers = null;
...
//        return taskValue + taskValue * effectivePrivacyBudget - alpha * realDistance - beta * privacyBudgetCost;
    ...
    public void initializeBasicInformation(List<Point> taskPositionList, Double[] taskValueArray, List<Point> workerPositionList, List<Double>[] privacyBudgetListArray) {
        ...
        this.workers = new SingleTaskPrivacyBasicWorker[workerPositionList.size()];
        for (int i = 0; i < workers.length; i++) {
            workerPosition = workerPositionList.get(i);
            this.workers[i] = new SingleTaskPrivacyBasicWorker(new double[]{workerPosition.getxIndex(), workerPosition.getyIndex()});
