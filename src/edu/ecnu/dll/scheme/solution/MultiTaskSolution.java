...
public class MultiTaskSolution {
    ...
    private void setCandidateTaskByBudget(List<Integer> tempCandidateTaskList, MultiTaskBasicWorker worker) {
...
//        while (!candidateWorkerIDList.isEmpty()) {
            ...
            for (int k = 0; k < oldCandidateWorkerIDList.length; k++) {
                ...
                for (Integer i : oldCandidateWorkerIDList[k]) {
                    ...
                    this.workers[i].alreadyPublishedEverageNoiseDistance[maxIncrementUtilityInfo.taskID] = maxIncrementUtilityInfo.noiseAverageDistance;
            ...
            for (int i = 0; i < newCandidateWorkerIDList.length; i++) {
                ...
                for (Integer j : newCandidateWorkerIDList[i]) {
                    competeTemp = LaplaceProbabilityDensityFunction.probabilityDensityFunction(this.workers[j].alreadyPublishedEverageNoiseDistance[i], taskCurrentWinnerInfoArray[i][DISTANCE_TAG], this.workers[j].alreadyPublishedTotalPrivacyBudget[i], taskCurrentWinnerInfoArray[i][BUDGET_TAG]);
                    if (competeTemp > 0.5) {
                        taskCurrentWinnerIDArray[i] = j;
                        taskCurrentWinnerInfoArray[i][DISTANCE_TAG] = this.workers[j].alreadyPublishedEverageNoiseDistance[i];
                        taskCurrentWinnerInfoArray[i][BUDGET_TAG] = this.workers[j].alreadyPublishedTotalPrivacyBudget[i];
                    }
                }
            }
        }
...
//        System.out.println("The winner worker's budget is " + taskTempWinnerInfo[1]);
     ...
     */
    private TaskIDDistanceBudgetPairTaskEntropy chooseByTaskEntropy(List<Integer> taskIDList, MultiTaskBasicWorker worker, Integer[] totalCompetingTimesList, int[] lastTermTaskWinnerIDArray, double[][] lastTermTaskWinnerInfoArray, HashSet<Integer>[] competingWorkerIDSet) {
